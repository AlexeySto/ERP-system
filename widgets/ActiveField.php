...
use app\assets\AirDatepicker3Asset;
use app\assets\InputMaskAsset;
use app\behaviors\MediaBehavior;
use app\models\MediaFile;
use app\theme\skote\assets\BootstrapDatepicker;
use Yii;
...
class ActiveField extends \yii\widgets\ActiveField
{
    ...
    public function date($options = [], $useTime = false)
    {
        AirDatepicker3Asset::register(Yii::$app->view);
        InputMaskAsset::register(Yii::$app->view);
        ...
        if ($value) {
            if ($useTime) {
                $selectedDates = "'" . Yii::$app->formatter->asDate($value, 'php:Y-m-d H:i') . "'";
            } else {
                $selectedDates = "'" . Yii::$app->formatter->asDate($value, 'php:Y-m-d') . "'";
            }
        }
        $options['autocomplete'] = 'off';
        $options['data-inputmask-regex'] = '^([0-2][0-9]|3[0-1])\.(0[0-9]|1[0-2])\.([0-9]{4})$';
